...
router.post('/chat', Auth.authenticateToken, async (req, res) => {
    try {
        ...
        try {
            const chatQuery = "INSERT INTO ai_chat (user_id, user_message, ai_response, context) VALUES (?, ?, ?, ?)";
            await db.query(chatQuery, [userId, userMessage, replyText, JSON.stringify(contextData || {})]);
        } catch (dbError) {
            console.warn('Failed to store chat in database:', dbError.message);
        }
...
router.get('/chat-history', Auth.authenticateToken, async (req, res) => {
    try {
        ...
        const chatQuery = "SELECT * FROM ai_chat WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?";
        const chatHistory = await db.query(chatQuery, [userId, parseInt(limit), parseInt(offset)]);

        // Get total count
        const countQuery = "SELECT COUNT(*) as total FROM ai_chat WHERE user_id = ?";
        const countResult = await db.query(countQuery, [userId]);
        ...
        try {
            ...
            if (jsonMatch) {
                try {
                    ...
                    console.error('Failed to parse JSON from Gemini response:', fallbackError);
                }
            }
        }
